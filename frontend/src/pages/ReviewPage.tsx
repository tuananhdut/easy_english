...
import { startReviewSession, checkReviewAnswer, getPaginatedScores } from '../features/study/studyApi'
...
const ReviewPage: React.FC = () => {
  ...
  const startSession = async () => {
    try {
      setLoading(true)
      const response = await startReviewSession({ collectionId: collectionId || '' })
      if (response.status === 'success' && response.data) {
      ...
      message.error('Không thể bắt đầu phiên ôn tập')
  ...
  const handleNext = async (value: string = '') => {
    ...
    try {
      ...
      if (status === 'introduction') {
        const response = await checkReviewAnswer(Number(collectionId), session.flashcards[currentIndex].term)
        if (response.status === 'success' && response.data) {
      ...
      if (status === 'quiz') {
        const response = await checkReviewAnswer(Number(collectionId), value)
        if (response.status === 'success' && response.data) {
          setSession(response.data.nextPhase)
          setStatus(response.data.nextPhase.status)
          setCurrentIndex(response.data.nextPhase.currentIndex)
      ...
      console.error('Error checking review answer:', err)
      message.error('Có lỗi xảy ra khi kiểm tra câu trả lời ôn tập')
    } finally {
      setLoading(false)
    }
  }
    ...
    return (
      <Layout style={{ minHeight: '100vh', background: '#2e3b55' }}>
        <Row justify='center' align='middle' style={{ minHeight: '100vh' }}>
          <Spin size='large' tip='Đang tải phiên ôn tập...'></Spin>
        </Row>
      </Layout>
    )
  }
